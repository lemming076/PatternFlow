...
wandb.init(
    project="FracNets",
    group="SierNet-Drop-Acc-Wiener",
    config={
        "id": experiment_id,
        "machine": "Xeon-3090-RTX",
        "learning_rate": learning_rate,
        "architecture": "SierNet-2",
        "dataset": "CIFAR-10",
        "epochs": epochs,
        "optimizer": "sgd",
        "loss": "crossentropy",
        "weight_decay": decay,
        "lrsched_pct_start": pct_start,
        "lrsched_pct_final": pct_final,
        "lrsched_div_factor": div_factor,
        "lrsched_final_div_factor": final_div_factor,
        "metric": "accuracy",
...
def _forward(batch: Batch, is_training: bool) -> jnp.ndarray:
    ...
    # cnn = hk.nets.ResNet34(num_classes, resnet_v2=False, bn_config={'decay_rate': 0.9})
...
# lr_schedule = optax.cosine_decay_schedule(1e-3, num_train_steps, 0.001)
...
for step in range(num_train_steps):
    if step % 100 == 0:
        ...
        wandb.log({"Step":step, "Train Acc":train_accuracy, "Test Acc":test_accuracy, "loss":loss})
...
wandb.run.summary["training_time"] = elapsed/60
...
wandb.run.summary["final_accuracy"] = correct.item() / total
wandb.log({"final_accuracy": correct.item() / total})
wandb.run.summary["testing_time"] = elapsed/60

#log plots of training
#scatterplots
# table = wandb.Table(data=train_data, columns = ["steps", "accuracy"])
# wandb.log({"Training Accuracy" : wandb.plot.scatter(table, "steps", "accuracy")})
# table = wandb.Table(data=test_data, columns = ["steps", "accuracy"])
# wandb.log({"Testing Accuracy" : wandb.plot.scatter(table, "steps", "accuracy")})
#matrices
# cm = wandb.plot.confusion_matrix(y_true=ground_truth, preds=predictions, class_names=class_names)
